...
helm install $functionAName ./charts/webapiapp -n $nameSpace --set ingress.useregex="false" --set ingress.path="/{{ .Release.Name }}(/|$)(.*)" --set ingress.rewritepath="/\$2"    --set deployment.image.repository=$acrDNSName --set deployment.image.imageName=$imageName  --set deployment.image.tag=$imageTag  --set deployment.imagePullSecrets[0]."name"="$acrPassword"   --set deployment.deploymentAnnotations."prometheus\.io/scrape"="true" --set deployment.deploymentAnnotations."prometheus\.io/port"="80" --set deployment.deploymentAnnotations."prometheus\.io/path"="/metrics"
