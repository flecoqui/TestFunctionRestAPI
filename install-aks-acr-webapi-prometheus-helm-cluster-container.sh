#!/bin/bash
##########################################################################################################################################################################################
#- Purpose: Script is used to deploy Azure Container Registry, Azure Key Vault, Azure kubernetes Cluster and deploy nginx, keda, prometheus and webapi containers
#- Parameters are:
...
helm install $functionAName ./charts/webapiapp -n ingress-nginx --set ingress.useregex="false" --set ingress.path="/{{ .Release.Name }}(/|$)(.*)" --set ingress.rewritepath="/\$2"    --set deployment.image.repository=$acrDNSName --set deployment.image.imageName=$imageName  --set deployment.image.tag=$imageTag  --set deployment.imagePullSecrets[0]."name"="$acrPassword"   --set deployment.deploymentAnnotations."prometheus\.io/scrape"="true" --set deployment.deploymentAnnotations."prometheus\.io/port"="80" --set deployment.deploymentAnnotations."prometheus\.io/path"="/metrics"

WriteLog "Deploying WebAPI Net Core 3.1 container hosting the function B" 
helm install $functionBName ./charts/webapiapp -n ingress-nginx --set ingress.useregex="false" --set ingress.path="/{{ .Release.Name }}(/|$)(.*)" --set ingress.rewritepath="/\$2"    --set deployment.image.repository=$acrDNSName --set deployment.image.imageName=$imageName  --set deployment.image.tag=$imageTag  --set deployment.imagePullSecrets[0]."name"="$acrPassword"   --set deployment.deploymentAnnotations."prometheus\.io/scrape"="true" --set deployment.deploymentAnnotations."prometheus\.io/port"="80" --set deployment.deploymentAnnotations."prometheus\.io/path"="/metrics"
